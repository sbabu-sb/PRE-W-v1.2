...
function adjudicateLine(
  ...
  if (remainingAllowed > 0 && !procedure.isPreventive) {
    ...
    // FIX: Corrected typo from d.remainingAllowed to d.allowedRemaining
...
export function calculateCombinedEstimate(
  ...
  for (let i = 0; i < payers.length; i++) {
    ...
    for (const [orderIndex, procState] of ordered.entries()) {
      ...
      // *** START REFACTORED COB LOGIC ***
      let actualPayerPayment: number;
      let actualPatientShareThisPayer: number;
      let steps: BreakdownStep[];

      if (i === 0) { // Primary Payer
        actualPayerPayment = asIf.payerPayment;
        actualPatientShareThisPayer = asIf.patientCostShare;
        steps = [
          ...preNotes,
          ...pricing.notes.map((n) => ({ description: "Pricing Adjustment", patientOwes: 0, notes: n } as BreakdownStep)),
          ...asIf.steps,
        ];
      } else { // Secondary+ Payer (COB Logic)
        // Run adjudication as if this payer were primary, using its OWN base accumulators for a clean calculation.
        const asIfPrimary = adjudicateLine(
          originalProcedure,
          payer.benefits,
          benMap.get(procState.id) || null,
          payer.patientAccumulators, // Use original, not running accs
          payer.familyAccumulators,
          finalAllowed,
          isOON
        );
        
        const method = resolveCobMethod(payer);
        ...
        actualPayerPayment = computeCobPayment({
            ...
            secondaryAllowed: finalAllowed,
        });

        // This payer's direct contribution to patient share is zero, as it only reduces an existing balance.
        actualPatientShareThisPayer = 0;

        // Use the "as if primary" steps for the breakdown, then add a final COB adjustment note.
        steps = [
          ...preNotes,
          ...pricing.notes.map((n) => ({ description: "Pricing Adjustment", patientOwes: 0, notes: n } as BreakdownStep)),
          ...asIfPrimary.steps,
        ...
        const cobAdjustment = cents(asIfPrimary.payerPayment - actualPayerPayment);
        if (Math.abs(cobAdjustment) > 0.001) {
            steps.push({
                description: "COB Adjustment",
                patientOwes: 0,
                notes: `Payment adjusted by $${-cobAdjustment.toFixed(2)} based on ${method} rules.`
            });
        }
      }
      // *** END REFACTORED COB LOGIC ***
      ...
      if (i === 0) {
        // Only primary payer determines write-offs and OON balance bills.
        if (isOON) {
      ...
      payerBlock.procedureEstimates.push({
        ...
        cptCode: originalProcedure.cptCode,
        originalBilledAmount: procState.originalBilledAmount,
        patientCostShare: actualPatientShareThisPayer,
      ...
      payerBlock.totalPatientShareThisPayer = cents(payerBlock.totalPatientShareThisPayer + actualPatientShareThisPayer);
    }
