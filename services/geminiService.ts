...
import { EstimateData, AiEstimate, Procedure, Payer, MetaData, VerificationResult, PaymentLikelihood, NpiData, BundlingAuditData, NecessityDetails, SearchResults, RiskFactor, AISummary, CaseNote, CaseAttachment, AIReminderSuggestion, AIGroupingSuggestion, SortKey } from "../types";
...
export const fetchGroupingSuggestions = async (
  worklistSummary: { total: number; statuses: Record<string, number>; authStates: Record<string, number> }
): Promise<{ success: boolean; data?: AIGroupingSuggestion[]; error?: string }> => {
  const systemPrompt = `You are a world-class Revenue Cycle Management (RCM) operations analyst AI. Your task is to suggest the most effective way to group a worklist to identify and resolve revenue risks. Based on the provided data summary, return 2-3 optimal grouping strategies as a valid JSON array. Each suggestion must include a 'path' (an array of valid grouping keys: 'status', 'primaryPayer', 'assignedTo', 'preServiceClearance', 'authStatus', 'dos') and a concise 'rationale' explaining WHY this grouping is useful *right now*.`;

  const userQuery = `Analyze this worklist summary and provide grouping suggestions: ${JSON.stringify(worklistSummary)}`;
    ...
    items: {
      ...
      properties: {
        path: {
          ...
          description: "An array of 1 to 3 valid SortKey strings for grouping.",
        },
        rationale: {
          type: Type.STRING,
          description: "A concise, user-friendly explanation for the suggestion.",
        },
      },
  ...
  return generateJson<AIGroupingSuggestion[]>(userQuery, systemPrompt, schema);
};

export const suggestReminderFromNote = async (noteContent: string): Promise<{ success: boolean; data?: AIReminderSuggestion; error?: string }> => {
    const systemPrompt = `You are an AI assistant for a healthcare revenue cycle specialist. Your task is to analyze a note and suggest a concise, actionable reminder if it contains a clear follow-up task. If no clear task is present, return null for all fields. Today's date is ${new Date().toLocaleDateString('en-CA')}. Your response MUST be a valid JSON object matching the provided schema. The 'dueAt' should be an ISO string for a reasonable future time (e.g., tomorrow at 10 AM, in 2 hours). The title should be a brief action phrase.`;
    
    const userQuery = `Analyze this note: "${noteContent}"`;

    const schema = {
        type: Type.OBJECT,
        properties: {
            title: { type: Type.STRING, description: "A concise title for the reminder (e.g., 'Call Aetna back'). Null if no task found." },
            dueAt: { type: Type.STRING, description: "The suggested due date and time in ISO 8601 format. Null if no task found." },
            rationale: { type: Type.STRING, description: "A brief explanation of why this reminder was suggested." },
            confidence: { type: Type.NUMBER, description: "Confidence score (0-1) in the suggestion." }
        },
    };

    const result = await generateJson<AIReminderSuggestion>(userQuery, systemPrompt, schema);

    // Don't return low-confidence or empty suggestions
    if (result.success && result.data && (!result.data.title || result.data.confidence < 0.7)) {
...
1.  **Sequential Processing:** Adjudicate payers strictly in their given order (Primary, then Secondary, etc.).
...
6.  **Coordination of Benefits (COB):** For secondary and tertiary payers, you must apply the "lesser of" patient liability rule.
    a. The starting claim balance for the secondary payer is the patient's cost-share remaining after the primary payer's adjudication.
    b. Calculate what the patient's cost-share *would have been* if the secondary plan were their only plan (i.e., run a full adjudication as if it were primary).
    c. The final patient responsibility for the service is the **lesser** of the amount from step (a) and the amount from step (b).
    d. The secondary payer's payment is the amount from step (a) minus the final patient responsibility from step (c).
...
export const generateAiClinicalScenario = async () => {
    const systemPrompt = `You are an expert medical data simulator. Your task is to generate a single, plausible, common clinical scenario for an outpatient service in the US. The response MUST be a valid JSON object matching the provided schema. Do not invent obscure or rare conditions. Stick to common scenarios like an office visit for a cold, a simple imaging study, or a routine lab test.`;

    const userQuery = `Generate a new clinical scenario. Pick a random common procedure.`;

    const schema = {
        type: Type.OBJECT,
        properties: {
            cptCode: { type: Type.STRING, description: "A common CPT code for the service." },
            cptDescription: { type: Type.STRING, description: "The official short description for the CPT code." },
            icdCode: { type: Type.STRING, description: "A plausible ICD-10 code that supports the CPT code." },
            icdDescription: { type: Type.STRING, description: "The description for the ICD-10 code." },
            clinicalNotes: { type: Type.STRING, description: "A brief, 1-2 sentence clinical note explaining the patient's presentation, suitable for an auth request." },
        },
        required: ["cptCode", "cptDescription", "icdCode", "icdDescription", "clinicalNotes"]
    };
    ...
    }>(userQuery, systemPrompt, schema, 'gemini-2.5-flash');
};

export const suggestCodesFromNote = async (clinicalNote: string) => {
    const systemPrompt = `You are an expert medical coder AI. Given a clinical note, extract the most likely CPT and ICD-10 codes. Provide up to 2 of each. Your response MUST be a valid JSON object matching the provided schema. Focus on common outpatient codes.`;

    const userQuery = `Analyze this clinical note and suggest codes: "${clinicalNote}"`;

    const codeSchema = {
        type: Type.OBJECT,
        properties: {
            code: { type: Type.STRING, description: "The suggested code." },
            desc: { type: Type.STRING, description: "The official short description of the code." },
        }
    };
    ...
    const schema = {
        type: Type.OBJECT,
        properties: {
            procedures: { type: Type.ARRAY, items: codeSchema },
            diagnoses: { type: Type.ARRAY, items: codeSchema },
        },
    };
    ...
    if (noteLower.includes('knee') && (noteLower.includes('pain') || noteLower.includes('osteoarthritis'))) {
        diagnoses.push({ code: 'M17.11', desc: 'Unilateral primary osteoarthritis, right knee' });
    }
    if (noteLower.includes('office visit') || noteLower.includes('visit')) {
        procedures.push({ code: '99213', desc: 'Office visit, est. patient, low complexity' });
    }
    if (noteLower.includes('x-ray') || noteLower.includes('imaging')) {
        procedures.push({ code: '73560', desc: 'Radiologic exam, knee, 2 views' });
    }
    if (procedures.length === 0 && diagnoses.length === 0) {
        // Default fallback if no keywords match
        procedures.push({ code: '99214', desc: 'Office visit, est. patient, moderate complexity' });
        diagnoses.push({ code: 'R07.9', desc: 'Chest pain, unspecified' });
    }

    const data = { procedures, diagnoses };
    return { success: true, data };
};

export const fetchAiCaseSummary = async (
    caseId: string,
    notes: CaseNote[],
    attachments: CaseAttachment[]
): Promise<{ success: boolean, data?: AISummary, error?: string }> => {
    // This is a simulated call. In a real app, this would involve a Gemini API call.
    await sleep(1500);

    try {
        if (notes.length < 3) {
            return { success: true, data: undefined }; // Don't generate for short threads
        }

        const lastNote = notes[notes.length - 1];
        const summaryText = `The case is currently blocked pending clinical documentation for the primary CPT. ${lastNote.authorName} is following up with the provider's office. An eligibility issue with the secondary payer was also noted and is being investigated by the @Eligibility Team.`;
        ...
        const summary: AISummary = {
            ...
            summary: summaryText,
            actions: [
                { label: "Obtain clinicals from provider", owner: "Maria Garcia", confidence: 0.95 },
                { label: "Re-verify secondary eligibility", owner: "Eligibility Team", confidence: 0.90 },
            ],
            sentiment: 'blocked',
            rationale: `Summary generated based on ${notes.length} notes and ${attachments.length} attachments, highlighting recent blockers.`,
            sourceItemIds: notes.map(n => n.id),
        };
